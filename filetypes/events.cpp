...
EventlistError Property::read(std::istream& file, int offset) {
	...
	fptr.seekg(Property::offset, std::ios::beg);
	name.resize(0x20);
...
EventlistError Action::read(std::istream& file, int offset) {
	...
	fptr.seekg(Action::offset, std::ios::beg);
	name.resize(0x20);
	if(!file.read(&name[0], 0x20)) {
		return EventlistError::REACHED_EOF;
	}
	...
	for (int i = 0; i < 3; i++) {
		int32_t flag_id;
		fptr.seekg(Action::offset + 0x28 + i * 4, std::ios::beg);
	...
	fptr.seekg(Action::offset + 0x34, std::ios::beg);
...
Property& Action::get_prop(std::string prop_name) {
	for (Property& prop : properties) {
		if (prop.name == prop_name) {
			return prop;
		}
	}
	//property with the name doesn't exist, this would only be caused by typos in the code though
}
...
EventlistError Actor::read(std::istream& file, int offset) {
	...
	fptr.seekg(Actor::offset, std::ios::beg);
	name.resize(0x20);
	if(!file.read(&name[0], 0x20)) {
		return EventlistError::REACHED_EOF;
	}
...
std::variant<EventlistError, Action*> Actor::add_action(FileTypes::EventList* list, std::string name, std::vector<Prop> properties) {
	Action action;
	action.name = name;
	std::variant<EventlistError, int> flag_id_to_set = list->get_unused_flag_id();
	if(flag_id_to_set.index() == 0) {
		return EventlistError::NO_UNUSED_FLAGS_TO_USE;
	}
	action.flag_id_to_set = std::get<1>(flag_id_to_set);
...
EventlistError Event::read(std::istream& file, int offset) {
	...
	for (int i = 0; i < 0x14; i++) {
		int32_t actor_index;
		fptr.seekg(Event::offset + 0x2C + i * 4, std::ios::beg);
		if(!file.read((char*)&actor_index, 4)) {
			return EventlistError::REACHED_EOF;
		}
		Utility::byteswap_inplace(actor_index);
	...
	fptr.seekg(Event::offset + 0x7C, std::ios::beg);
	...
	for (int i = 0; i < 2; i++) {
		int32_t flag_id;
		fptr.seekg(Event::offset + 0x80 + i * 4, std::ios::beg);
		if(!file.read((char*)&flag_id, 4)) {
			return EventlistError::REACHED_EOF;
		}
		Utility::byteswap_inplace(flag_id);
	...
	for (int i = 0; i < 3; i++) {
		int32_t flag_id;
		fptr.seekg(Event::offset + 0x88 + i * 4, std::ios::beg);
		if(!file.read((char*)&flag_id, 4)) {
			return EventlistError::REACHED_EOF;
		}
		Utility::byteswap_inplace(flag_id);
	...
	fptr.seekg(Event::offset + 0x94, std::ios::beg);
	if(!file.read((char*)&play_jingle, 1)) {
		return EventlistError::REACHED_EOF;
	}

	fptr.seekg(Event::offset + 0x95, std::ios::beg);
...
Actor& Event::get_actor(std::string name) {
	for (Actor& actor : actors) {
		if (actor.name == name) {
			return actor;
		}
	}
	//Exception that i dont know how to handle
}

std::variant<EventlistError, Actor*> Event::add_actor(FileTypes::EventList* list, std::string name) {
	Actor actor;
	actor.name = name;
	std::variant<EventlistError, int> flag_id_to_set = list->get_unused_flag_id();
	if (flag_id_to_set.index() == 0) {
	...
	actor.flag_id_to_set = std::get<1>(flag_id_to_set);
...
namespace FileTypes
{
	...
	EventlistError EventList::loadFromBinary(std::istream& file_entry) {
		...
		unsigned int offset = string_list_offset;
		while (offset < string_list_offset + string_list_total_size) {
			std::variant<EventlistError, std::string> string = read_str_until_null_character(file_entry, offset);
			if (string.index() == 0) {
				return std::get<0>(string);
			}
			All_Strings_By_Offset[offset - string_list_offset] = std::get<1>(string);
			int string_length_with_null = std::get<1>(string).length() + 1;
	...
	EventlistError EventList::writeToStream(std::ostream& file_entry) {
		...
		for (int i = 0; i < num_properties; i++) {
			...
			if (property_value.index() != 3) {
				...
				property.data_index = NULL;
				property.data_size = NULL;
		...
		for (Property& property : All_Properties) {
			if (property.data_type == 4) {
				...
				int new_relative_string_offset = offset - string_list_offset;

				int string_length_with_null = string.length() + 1;
				offset = offset + string_length_with_null;

				int string_length_with_padding = string_length_with_null;
				if (string_length_with_null % 8 != 0) {
	...
	std::variant<EventlistError, int> EventList::get_unused_flag_id() {
