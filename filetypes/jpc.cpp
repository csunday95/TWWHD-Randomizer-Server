...
namespace chunks {
	...
	void TEX1_read(TEX1& tex, const chunk& chunk) {
		...
		strncpy_s(&tex.filename[0], str_length, &chunk.data[0xC], str_length);
		return;
	}
...
JPCError Particle::save_changes(int num_textures) {
...
namespace FileTypes {
	...
	void JPC::initNew() {
		memcpy_s(magicJPAC, 8, "JPAC1-00", 8);
		num_particles = 0;
		num_textures = 0;
	...
	JPCError JPC::loadFromBinary(std::istream& jpc) {
		...
		for (int tex_index = 0; tex_index < num_textures; tex_index++) {
			...
			tex.magic.resize(4);
			strncpy_s(&tex.magic[0], 4, chunk_magic, 4);
	...
	JPCError JPC::writeToStream(std::ostream& out) {
		...
		for (Particle& particle : particles) {
			TDB1 tdb1; //Instead of storing all the chunks as their unique classes, we store their data, so it all needs to be read into the right variables here
			chunks::TDB1_read(tdb1, particle.chunks_by_type["TDB1"]);
			Utility::byteswap_inplace(num_textures); //Swap this back so we can use it
			tdb1.texture_ids = read_texture_ids(particle.chunks_by_type["TDB1"], num_textures);
			tdb1.texture_filenames.reserve(tdb1.texture_ids.size());
			for (const uint16_t& id : tdb1.texture_ids) {
				TEX1 tex;
				chunks::TEX1_read(tex, textures[id]);
				tdb1.texture_filenames.push_back(tex.filename);
			}

			for (const std::string& texture_filename : tdb1.texture_filenames) {
				...
				tdb1.texture_ids.push_back(texture_id);
			}

			chunks::TDB1_save(tdb1, particle.chunks_by_type["TDB1"]);
			particle.save_changes(tdb1.texture_ids.size());
