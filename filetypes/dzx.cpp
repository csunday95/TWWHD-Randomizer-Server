...
DZXError Chunk::read(std::istream& file, int offset) {
	...
	if (strncmp("TRE", type, 3) == 0) {
		...
		if (LAYER_CHAR_TO_LAYER_INDEX.find(layer_char) != LAYER_CHAR_TO_LAYER_INDEX.end()) {
			layer = LAYER_CHAR_TO_LAYER_INDEX[layer_char];
		}
...
namespace FileTypes {
	...
	ChunkEntry& DZXFile::add_entity(char chunk_type[4], int layer) {
		...
		if (chunk_to_add_to == nullptr) {
			Chunk chunk;
			memcpy(chunk.type, chunk_type, 4);
			chunk.layer = layer;
			chunk.entry_size = size_by_type[chunk_type];
			chunk.entries.push_back(entity);
			chunks.push_back(chunk);
